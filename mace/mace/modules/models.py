...
import logging
from .ewald_mace import EwaldPotential
...
@compile_mode("script")
class MACE_original(torch.nn.Module):
    def __init__(
        self,
        r_max: float,
        num_bessel: int,
...
class MACE(torch.nn.Module):
    def __init__(
        ...
        print("in the class definition:", num_interactions)
        ...
        for i in range(num_interactions - 1):
            ...
            print("in the linear readout block", num_interactions, hidden_irreps_out, MLP_irreps, len(heads))
        
        ewald_node_feat_dimension_in = 1024
        self.ewald_readout = LinearReadoutBlock(o3.Irreps(f"{ewald_node_feat_dimension_in}x0e"), o3.Irreps("256x0e"))     
    ...
    def forward(
        ...
        for interaction, product, readout in zip(
            ...
            node_feats_list.append(node_feats)
            logging.info(f"node_feats: {node_feats.shape}")
            node_energies = readout(node_feats, node_heads)[
                num_atoms_arange, node_heads
            ]  # [n_nodes, len(heads)]
            energy = scatter_sum(
